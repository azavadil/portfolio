...
/** 
 * Instance method: performFetch 
 * --------------------
 * performFetch causes the fetchedResultsController to refetch the data.
 * You almost certainly never need to call this. The NSFetchedResultsController 
 * class observes the context (hence, if objects in the context change, you do not need 
 * to explicitly call performFetch. The NSFetchedResultsController will notice and update 
 * the table automatically). This will also automatically be called if you change the 
